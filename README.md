...
<p>`'message'` — строка:
 <a>`Success` - отправлено успешно;</a>
 <a>`Причина ошибки` (если она была);</a>
<p>`'id'` —  идентификатор, который был присвоен объекту при добавлении в базу данных при `status=200`.</p>
